...
abstract contract Manageable is Context {
    ...
    event ControlTransferred(address indexed Controller, address indexed newController);
   constructor(){
        address msgSender = _msgSender();
        _manager = msgSender;
        _controller = msgSender;
        _protector = msgSender;
     ...
     * @param newController The address allowed to burn/mint tokens to control supply.
     */
    function setController(address newController) public onlyController {
        require(newController != address(0), "cannot set supply controller to address zero");
        emit ControlTransferred(_controller, newController);
        _controller = newController;
    }

    modifier onlyController() {
        require(msg.sender == _controller, "only Controller or Owner");
        _;
    }
...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "SafeTest";
    string internal constant SYMBOL = "SAFETEST";
    ...
    uint256 internal TOTAL_SUPPLY = 1000000 * ZEROES;
     ...
     */
    uint256 internal  maxTransactionAmount = TOTAL_SUPPLY * 10**6 / 100; // 1% of the total supply
    ...
    address internal marketingAddress = 0x62719b79DD17EfE2CD18A2EE6B32125CfD680B90;
    address internal developerAddress = 0x01785d937D2AC6fe6c2ee4ccb2a66F30b91383BF;
    ...
    function _addFee(FeeType feeName, uint256 value, address recipient) private {
        fees.push( Fee(feeName, value, recipient, 0 ) );
        sumOfFees += value;
    }
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presaleable, Tokenomics {
    ...
    address _protector = msg.sender;
    address _controller = msg.sender;
    address public mktgFeeRecipient;
    ...
    event MktgFeeRecipientSet(
        address indexed oldMktgFeeRecipient,
        address indexed newMktgFeeRecipient
    );
    event DevFeeRecipientSet(
        address indexed oldDevFeeRecipient,
        address indexed newDevFeeRecipient
    );
    ...
    /** Functions required by IERC20 **/
        ...
        function increaseSupply(uint256 amount) public onlyController returns (bool success) {
        TOTAL_SUPPLY = TOTAL_SUPPLY + (amount * 10**6);
        _balances[_controller] = _balances[_controller] + (amount * 10**6);
        emit SupplyIncreased(_controller, amount);
        emit Transfer(address(0), _controller, amount);
        return true;
    }


        function decreaseSupply(uint256 amount) public onlyController returns (bool success) {
        require(amount <= _balances[_controller], "not enough supply");
        _balances[_controller] = _balances[_controller] - (amount * 10**6);
        TOTAL_SUPPLY = TOTAL_SUPPLY - (amount*10**6);
        emit SupplyDecreased(_controller, amount);
        emit Transfer(_controller, address(0), amount);
        return true;
    }
    
        function setDevFeeRecipient(address _newDevFeeRecipient) public onlyController {
        require(_newDevFeeRecipient != address(0), "cannot set fee recipient to address zero");
        address _oldDevFeeRecipient = devFeeRecipient;
        devFeeRecipient = _newDevFeeRecipient;
        emit DevFeeRecipientSet(_oldDevFeeRecipient, devFeeRecipient);
    }

    /**
     * @dev Sets a new fee recipient address.
     * @param _newMktgFeeRecipient The address allowed to collect transfer fees for transfers.
     */

        function setMktgFeeRecipient(address _newMktgFeeRecipient) public onlyController {
        require(_newMktgFeeRecipient != address(0), "cannot set fee recipient to address zero");
        address _oldMktgFeeRecipient = mktgFeeRecipient;
        mktgFeeRecipient = _newMktgFeeRecipient;
        emit MktgFeeRecipientSet(_oldMktgFeeRecipient, mktgFeeRecipient);
    }
